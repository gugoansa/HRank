...
export class TablePage {
    ...
    private readonly tableRows: Locator = this.page.locator('#customers tr:not(:first-child)') ;  //#customers.ws-table-all
    public async getTableData(): Promise <string[][]> {
      ...
      console.log(
        `Simulacion: Playwright encontro ${rowCount} filas de objetos`
      );
      ...
      for (let i = 0; i < rowCount; i++) {
        ...
        const cellData: string[] = await rowLocator.locator("td").allTextContents();
